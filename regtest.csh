...
EOF
	...
	echo "Overriding architecture check for Docker container"
	set ARCH = Linux
	set MPIRUNCOMMAND = "mpirun -np 4"
	# skip unnecessary cluster checks, let script continue
endif
...
( cd /wrf/WRF/test/em_real; ln -sf $thedataem/* . ) 
( cd /wrf/WRF/test/nmm_real ; ln -s $thedatanmm/met_nmm* . ; \
  cp $thedatanmm/namelist.input.regtest . )
#DAVE###################################################
( cd /wrf/WRF/test/em_real ; ls -ls )
( cd /wrf/WRF/test/nmm_real ; ls -ls )
banner 4
#set ans = "$<"
#DAVE###################################################
